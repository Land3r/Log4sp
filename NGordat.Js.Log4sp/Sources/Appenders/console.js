 ...
 * @author Seth Chisamore - adapted for use as a log4sp appender
 */
Log4sp.ConsoleAppender = function(isInline) {
	
	/**
	 * @type Log4sp.Layout
	 * @private
	 */
	this.layout = new Log4sp.PatternLayout(Log4sp.PatternLayout.TTCC_CONVERSION_PATTERN);
	/**
	 * @type boolean
	 * @private
	 */
	...
	if(this.inline) {
		Log4sp.attachEvent(window, 'load', Log4sp.bind(this.initialize, this));
	}
};

Log4sp.ConsoleAppender.prototype = Log4sp.extend(new Log4sp.Appender(), /** @lends Log4sp.ConsoleAppender# */ {
  	...
  	initialize : function() {
		...
		if(!this.inline) {
			...
			if (!win) { 
				...
				alert("Popup window manager blocking the Log4sp popup window to bedisplayed.\n\n" 
				...
				doc.write("<html><head><title>Log4sp - " + this.logger.category + "</title>\n");
		...
		if(this.inline) {
			...
			closeButton.onclick = Log4sp.bind(this.toggle, this);
		...
		clearButton.onclick = Log4sp.bind(this.logger.clear, this.logger);
		...
		this.tagFilterContainerElement.appendChild(this.docReference.createTextNode("Log4sp - " + this.logger.category));
		...
		Log4sp.attachEvent(this.tagFilterElement, 'keyup', Log4sp.bind(this.updateTags, this));
		Log4sp.attachEvent(this.tagFilterElement, 'click', Log4sp.bind( function() {this.tagFilterElement.select();}, this));
		...
		Log4sp.attachEvent(this.inputElement, 'keyup', Log4sp.bind(this.handleInput, this));
		Log4sp.attachEvent(this.inputElement, 'click', Log4sp.bind( function() {this.inputElement.select();}, this));
		
		if(this.inline){
			window.setInterval(Log4sp.bind(this.repositionWindow, this), 500);
			...
			accessElement.onclick = Log4sp.bind(this.toggle, this);
	 ...
	 */
	 toString: function() {
	 	return "Log4sp.ConsoleAppender[inline=" + this.inline + "]"; 
	 }
}); 
