 ...
 * use {@see Log4sp#getLogger(String)} to get an instance.
 * @constructor
 * @param name name of category to log to
 * @author Stephan Strittmatter
 */
Log4sp.Logger = function(name) {
	this.loggingEvents = [];
	this.appenders = [];
	/** category of logger */
	this.category = name || "";
	/** level to be logged */
	this.level = Log4sp.Level.FATAL;
	
	this.dateformat = Log4sp.DateFormatter.DEFAULT_DATE_FORMAT;
	this.dateformatter = new Log4sp.DateFormatter();
	
	this.onlog = new Log4sp.CustomEvent();
	this.onclear = new Log4sp.CustomEvent();
	
	/** appender to write in */
	this.appenders.push(new Log4sp.Appender(this));
	...
	try {
		...
		//log4spLogger.fatal(e);
	}
};

Log4sp.Logger.prototype = {
	...
	addAppender: function(appender) {
		if (appender instanceof Log4sp.Appender) {
	...
	log: function(logLevel, message, exception) {
		var loggingEvent = new Log4sp.LoggingEvent(this.category, logLevel, 
	...
	isTraceEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.TRACE.valueOf()) {
			return true;
		}
		return false;
	},
	...
	trace: function(message) {
		if (this.isTraceEnabled()) {
			this.log(Log4sp.Level.TRACE, message, null);
		}
	},
	/** checks if Level Debug is enabled */
	isDebugEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.DEBUG.valueOf()) {
			return true;
		}
		return false;
	},
	...
	debug: function(message, throwable) {
		if (this.isDebugEnabled()) {
			this.log(Log4sp.Level.DEBUG, message, throwable);
		}
	},	
	/** checks if Level Info is enabled */
	isInfoEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.INFO.valueOf()) {
			return true;
		}
		return false;
	},
	...
	info: function(message, throwable) {
		if (this.isInfoEnabled()) {
			this.log(Log4sp.Level.INFO, message, throwable);
		}
	},
	/** checks if Level Warn is enabled */
	isWarnEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.WARN.valueOf()) {
			return true;
		}
		return false;
	},
	...
	warn: function(message, throwable) {
		if (this.isWarnEnabled()) {
			this.log(Log4sp.Level.WARN, message, throwable);
		}
	},
	/** checks if Level Error is enabled */
	isErrorEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.ERROR.valueOf()) {
			return true;
		}
		return false;
	},
	...
	error: function(message, throwable) {
		if (this.isErrorEnabled()) {
			this.log(Log4sp.Level.ERROR, message, throwable);
		}
	},
	/** checks if Level Fatal is enabled */
	isFatalEnabled: function() {
		if (this.level.valueOf() <= Log4sp.Level.FATAL.valueOf()) {
			return true;
		}
		return false;
	},
	...
	fatal: function(message, throwable) {
		if (this.isFatalEnabled()) {
			this.log(Log4sp.Level.FATAL, message, throwable);
		}
	},	
	/** 
	...
	windowError: function(msg, url, line){
		...
		this.log(Log4sp.Level.FATAL, message, null);	
	},
	...
	/**
	 * Generates a timestamp using the format set in {Log4sp.DateFormatter.formatDate}.
